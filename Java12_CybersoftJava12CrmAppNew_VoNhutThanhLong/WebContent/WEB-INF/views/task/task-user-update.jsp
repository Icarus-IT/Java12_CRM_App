		...
		<div class="card card-form">
			<div class="row no-gutters">
				...
				<div class="col-lg-8 card-form__body card-body">
					<form action="<c:url value="<%=UrlConst.TASK_USER_UPDATE%>" />"
						method="post">
						<div class="form-group">
							<label for="name">Name:</label> <input type="text" disabled="disabled"
						...
						<div class="form-group">
							<label for="name">Description:</label> <input type="text" disabled="disabled"
								value="${TaskToUpdate.description }" class="form-control"
								name="description" id="description" required>
						</div>
						<div class="form-group">
							<label for="name">Start date</label> <input type="date" disabled="disabled"
						...
						<div class="form-group">
							<label for="name">End date</label> <input type="date" id="end" disabled="disabled"
						...
						<label for="name">Status</label>
							<select name="cboStatus">
								<c:forEach var="sDto" items="${STATUSLIST}">

									
									<c:if
										test="${sDto.id eq TaskToUpdate.statusId}">
										<option selected="selected" value="${sDto.getId()}">${sDto.getName()}</option>
									</c:if>
									<c:if
											test="${sDto.id ne TaskToUpdate.statusId}">
										<option value="${sDto.getId()}">${sDto.getName()}</option>
									</c:if>
								</c:forEach>
							</select>
						</div>
