						...
						<c:otherwise>
							<c:forEach var="dto" items="${tasks}">
								<tr>
									...
									<td><a href="<c:url value="<%=UrlConst.TASK_UPDATE%>" />?tid=${dto.id}" class="text-muted"><i class="material-icons">settings</i></a>
										<a href="<c:url value="<%=UrlConst.TASK_DELETE%>" />?tid=${dto.id}&pid=${dto.getProject().getId()}" class="text-muted"><i class="material-icons">delete</i></a> 
									</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
